...
const Home = (props: HomeProps) => {
    ...
    const handleBuy = async (index : number) =>{
        try{
            ...
            await contract.methods.buyMusic().send({
                from: accounts[0],
                value: props.web3.utils.toWei(libraryDetails.current[index].price)
            })
        }
        catch(error){
    ...
    const loadMusic = async () => {
        try {
            ...
            const musicContracts = await props.factory.methods.getLibrary().call()
                ...
                {musicData.map((d, index) => (
                    <Card key={index} className={classes.card}>
                        ...
                        <CardActions>
                            ...
                            <Button size="small" onClick={() => handleBuy(index)} color="primary">
                                Buy
                        </Button>
                        </CardActions>
                    </Card>
