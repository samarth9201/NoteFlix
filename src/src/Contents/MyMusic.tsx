...
const MyMusic = (props: MyMusicProps) => {
    ...
    const handleWithdraw = async (index:number) =>{
        try{
            ...
            await libraryDetails.current[index].contract.methods.withdraw().send({
                from: accounts[0]
            })
        }
        catch(error){
    ...
    const loadMusic = async () => {
        try {
            ...
            const musicContracts = await userContract.methods.getMyMusic().call()
            ...
            for (let i = 0; i < musicContracts.length; i++) {
                ...
                const bal = await props.web3.eth.getBalance(musicContracts[i])
            ...
            const comp = <Container className={classes.root}>
                {musicData.map((d, index) => (
                    <Card key={index} className={classes.card}>
                        ...
                        <Button size="small" color="primary" onClick={() => handleWithdraw(index)}>
                                Withdraw
                        </Button>
                        </CardActions>
                    </Card>
